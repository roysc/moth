  ...
  _ctx(cx)
{
  ...
  for (auto&& c : cpts)
    _compts.emplace(c, _ctx->get_cptclass(c)->create());
  LOG_SHOW_TO_(_compts, lctor);
}

string Entity::get_name(Compt_id id) const {return _ctx->get_cptclass(id)->name();}

// O(n)
Compt_ptr Entity::find(const string& cn) 
{
  for (auto&& c: _compts)
    if (_ctx->get_cptclass(c.first)->name() == cn)
      return &c.second;
  return {};
}
